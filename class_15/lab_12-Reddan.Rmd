...
#### [Q5b]: You could also use the ggplot2 package to make this figure. What geom_?() function would you use for this plot?  
...
I do not trust these results since there is not accountability for genes with a high variance in gene expression data. Means do not represent the data well enough to rely on solely, statistics would be needed to identify with the fold change is significance or is observed by chance.  
...
```{r}
pathview(gene.data=foldchanges, pathway.id=substr(rownames(keggres$less)[1], start = 0, stop = 8))
pathview(gene.data=foldchanges, pathway.id=substr(rownames(keggres$less)[2], start = 0, stop = 8))
```
